...
* If the value in the `Id, Name` fields are empty then action insert a new record according input parameter entity record structure (`AccessGroupInsertData`).
...
Use the query string resource to delete `AccessGroup` record. The method contains two parameters `Id` and `Name`. Specify the record `{Id}`, `Name` using `Id, Name` field(s) value and use the `DELETE` method of the resource to delete a record.
...
Use the `AccessGroupUpdateData` object resource to update `AccessGroup` using `Id, Name` field(s) value.
...
Use the `AccessGroupData` object resource to insert or update (Upsert) `AccessGroup` using `Id, Name` field(s) value.
...
### Create a new record of AccessGroup
* If the value in the `Name` and `Id` fields are empty then action insert a new record according input parameter entity record structure (`AccessGroupData`).
